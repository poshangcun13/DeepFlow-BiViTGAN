...
# Create DataLoader for training data (only features, no labels)
def create_dataloader(feature_data, batch_size):
    # Convert feature_data to a torch tensor
    tensor_data = torch.Tensor(np.array(feature_data))
    # TensorDataset expects the data to be a tuple, even if there's no label
    dataset = TensorDataset(tensor_data)  # Only features, no labels
    # Create DataLoader
...
if __name__ == '__main__':
    ...
    # Prepare DataLoader for benign and attack training data (only features)
    train_dataloader_attack = create_dataloader(feature22, batch_size)
    train_dataloader_benign = create_dataloader(feature11, batch_size)

    # Train attack data model
    for epoch in range(num_epochs):
    # Display training progress bar
        for i, attack_data in tqdm(enumerate(train_dataloader_attack), total=len(train_dataloader_attack),
                                   desc=f"Training Attack Epoch {epoch + 1}/{num_epochs}"):
            attack_data = attack_data[0].to(device)
    ...
    for epoch in range(num_epochs):
        ...
        total_real = 0
        # Display training progress bar
        for i, benign_data in tqdm(enumerate(train_dataloader_benign), total=len(train_dataloader_benign),
                                   desc=f"Training benign Epoch {epoch + 1}/{num_epochs}"):
            benign_data = benign_data[0].to(device)
            attack_data = torch.Tensor(np.array(feature22[i % len(feature11)])).to(device)

            # Train generator Btrue
            optimizer_G_benign.zero_grad()
            generated_fake = generator_benign(attack_data)
            ...
            pred_fake = discriminator_benign(attack_data)  # Discriminator prediction of attack sample
            # Discriminator loss: Distinguish between attack data and benign data
            ...
            if i % 100 == 0:  # Output loss every 100 steps
                print(f"Step {i}, Loss G benign: {loss_G_benign.item():.4f}, Loss D benign: {loss_D_benign.item():.4f}")
    ...
    # Save the models
    # torch.save(generator_attack.state_dict(), "generator_CIC19_attack.pth")
    # torch.save(discriminator_attack.state_dict(), "discriminator_CIC19_attack.pth")
    # torch.save(generator_benign.state_dict(), "generator_CIC19_benign.pth")
    # torch.save(discriminator_benign.state_dict(), "discriminator_CIC19_benign.pth")